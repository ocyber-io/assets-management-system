...
type DeleteMultipleFoldersModalProps = {
  heading?: string;
  description?: string;
...
const DeleteMultipleFoldersModal: React.FC<DeleteMultipleFoldersModalProps> = ({
  heading = "Move to bin?",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showSuccessToast(
        `${folderIds.length} Folder(s) moved to bin successfully`
      );
      fetchFolders();
      fetchAllFiles();
      onClose();
    } catch (error: any) {
      showErrorToast(error || "An error occurred while moving the folders.");
    }
  };
...
export default DeleteMultipleFoldersModal;
